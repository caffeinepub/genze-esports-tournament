...
import { getTournaments, getPlayers } from '../utils/seedData';
...
function StatCard({ stat, started }: { stat: StatItem; started: boolean }) {
  ...
  return (
    <div className="stats-card">
      ...
      <div
        className="font-orbitron text-3xl font-800 mb-2"
        style={{ color: stat.color }}
      >
        {stat.prefix}{count.toLocaleString('en-IN')}{stat.suffix}
      </div>
      <div className="font-rajdhani text-sm font-600 tracking-wider uppercase" style={{ color: '#6b6b88' }}>
        {stat.label}
      </div>
    </div>
  );
