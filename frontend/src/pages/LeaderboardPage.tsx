...
export default function LeaderboardPage() {
  ...
  const filtered = filter === 'All' ? approvedPlayers : approvedPlayers.filter((p) => p.game === filter);
  ...
  return (
    <div style={{ background: '#0a0a0f', minHeight: '100vh' }}>
      ...
      >
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'url("/assets/generated/hero-banner.dim_1920x600.png")', backgroundSize: 'cover', backgroundPosition: 'center' }} />
        ...
        <div className="relative">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="section-heading text-3xl sm:text-4xl md:text-5xl" style={{ color: '#e8e8f0' }}>
              Leader<span className="neon-text-orange">board</span>
            </h1>
            <Trophy className="w-8 h-8" style={{ color: '#ffd700' }} />
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        {filtered.length > 0 ? (
          <div className="space-y-3">
            ...
            {filtered.length >= 3 && (
              <div className="grid grid-cols-3 gap-4 mb-8">
                {[filtered[1], filtered[0], filtered[2]].map((player, podiumIdx) => {
                  if (!player) return null;
                  const rank = podiumIdx === 0 ? 2 : podiumIdx === 1 ? 1 : 3;
                  const heights = ['h-28', 'h-36', 'h-24'];
                  const colors = ['#c0c0c0', '#ffd700', '#cd7f32'];
                  const t = tournaments.find((x) => x.id === player.tournamentId);
                  return (
                    <div key={player.id} className={`flex flex-col items-center justify-end ${heights[podiumIdx]}`}>
                      ...
                      >
                        <div className="font-orbitron font-700 text-xs mb-1" style={{ color: colors[podiumIdx] }}>
                          #{rank}
                        </div>
                        <div className="font-rajdhani font-600 text-sm truncate" style={{ color: '#e8e8f0' }}>
                          {player.fullName}
                        </div>
                        <div className="text-xs truncate" style={{ color: '#6b6b88' }}>
                  ...
                  {filtered.map((player, index) => {
                    ...
                    return (
                      <tr key={player.id}>
                        ...
                        <td>
                          <div className="font-600" style={{ color: '#e8e8f0' }}>{player.fullName}</div>
                          <div className="text-xs" style={{ color: '#6b6b88' }}>@{player.username}</div>
                        </td>
                        <td>
                          <span className={player.game === 'Free Fire' ? 'badge-ff' : 'badge-pubg'}>
                            {player.game === 'Free Fire' ? '🔥 FF' : '🪖 PUBG'}
                          </span>
                        </td>
                        <td style={{ color: '#8b8ba8' }}>{player.teamName || '—'}</td>
                        <td style={{ color: '#8b8ba8' }}>{t?.name || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div
            ...
            style={{ background: '#12121a', border: '1px solid #2a2a3a' }}
          >
            <Medal className="w-16 h-16 mx-auto mb-4" style={{ color: '#2a2a3a' }} />
